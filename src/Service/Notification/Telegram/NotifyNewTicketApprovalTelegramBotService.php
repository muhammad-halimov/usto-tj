...
class NotifyNewTicketApprovalTelegramBotService extends AbstractTicketApprovalNotificationService
{
    ...
    public function sendTicketApprovalNotification(User $user, TicketApproval $ticketApproval): mixed
    {
        ...
        // Подпись рядом с каждым значком (05.09.2026, по просьбе "добавь
        // слово описание рядом с иконкой, как с отсальными") — те же
        // подписи, что уже стояли у категории/бюджета/автора, теперь
        // единообразно у ВСЕХ строк, включая заголовок и описание — раньше
        // именно эти два были единственным исключением (голый значок без
        // слова), из-за чего выглядели инородно на фоне остальных.
        $message =
            ($isNew ? "🆕 <b>Новое объявление на проверку</b>\n" : "🔎 <b>Услуга/объявление на проверку</b>\n") .
            "{$divider}\n\n" .
            (!$isNew && $changes !== '—' ? "✏️ <b>Изменения:</b>\n{$changes}\n\n" : '') .
            "📌 Заголовок: <b>{$ticket?->getTitle()}</b>\n\n" .
            "📂 Категория: {$this->category($ticket)}\n" .
            "💰 Бюджет: {$this->budget($ticket)}\n" .
            "👤 Автор: {$this->owner($ticket)}\n\n" .
            "📝 Описание: {$desc}\n\n" .
            "{$divider}\n" .
            "🔗 Ссылка: <a href='{$adminUrl}'>Открыть в админке</a>";
