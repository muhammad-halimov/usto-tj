...
class AppMessages
{
    ...
    /**
     * Локаль текущего запроса (см. AppErrorLocaleListener) — единая точка,
     * которую переиспользуют и другие места, которым нужен "текущий язык
     * пользователя", а не только сообщения об ошибках (см.
     * TicketListener::geoRef()/onFlush() — раньше там была захардкожена
     * 'ru' вместо этого, из-за чего снимок адреса в TicketApproval/
     * EntityRevision всегда писался по-русски независимо от ?locale=
     * запроса, автора тикета и настроек сайта).
     */
